...
const QSize CloakEnemy::ms_fixedSize(52, 52);

CloakEnemy::CloakEnemy(WayPoint *startWayPoint, MainWindow *game, const QPixmap &sprite):
...
void CloakEnemy::doActivate()
{
    m_active = true;
}

void CloakEnemy::move()
{
    if (!m_active)
        return;
...
void CloakEnemy::draw(QPainter *painter) const
{
    if (!m_active)
        return;
...
void CloakEnemy::getDamage(int damage)
{
...
void CloakEnemy::getAttacked(PurpleTower *attacker)
{
    m_PurpleTowerattackedTowersList.push_back(attacker);
}
void CloakEnemy::getAttacked(BlueTower *attacker)
{
    m_BlueTowerattackedTowersList.push_back(attacker);
}
void CloakEnemy::getAttacked(RedTower *attacker)
{
    m_RedTowerattackedTowersList.push_back(attacker);
}

// 表明敌人已经逃离了攻击范围
void CloakEnemy::gotLostSight(PurpleTower *attacker)
{
    m_PurpleTowerattackedTowersList.removeOne(attacker);
}
void CloakEnemy::gotLostSight(BlueTower *attacker)
{
    m_BlueTowerattackedTowersList.removeOne(attacker);
}
void CloakEnemy::gotLostSight(RedTower *attacker)
{
    m_RedTowerattackedTowersList.removeOne(attacker);
}

void CloakEnemy::setCloak(bool state)
{
    m_cloak = state;
}

QPoint CloakEnemy::pos() const
{
    return m_pos;
}
